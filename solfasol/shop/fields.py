...
class ExpiryDateWidget(forms.MultiWidget):
    ...
    def format_output(self, rendered_widgets):
        return u'<div class="col">%s</div>' % ' '.join(rendered_widgets)
...
class ExpiryDateField(forms.MultiValueField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        today = date.today()
        months = [(x, '%02d' % x) for x in range(1, 13)]
...
class VerificationValueField(forms.CharField):
    ...
    default_error_messages = {
        'required': _(u'Please enter the verification code for your credit card.'),
